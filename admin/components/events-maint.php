...
$tablename = DB_TABLE_PREFIX.'events';
...
if ( $form_action != '' ) {
	...
	$delctr = 0;
	$deldatefrm = mysql_query("select * from tbl_event_dates where eevent_id='$event_id' and section=1 order by date_id asc");
	while ($delrows = mysql_fetch_array($deldatefrm)){
		...
		echo $eventstartdate = "20".$year."-".$month."-".dayconvert($day);
		
		$eventenddate = $_POST['event_end_date'.$delctr];
		list($day, $month, $year) = split('[/.-]', $eventenddate);
		echo $eventenddate = "20".$year."-".$month."-".dayconvert($day);
	...
	$event_start_datexx = date("Y-m-d H:i:s",strtotime($eventstartdate . ' ' . $event_start_time));
	$event_end_datexx = date("Y-m-d H:i:s",strtotime($eventenddate . ' ' . $event_end_time));
...
switch ($form_action)
{
	case 'ADD':		
		...
		if ($id > 0){
			...
			for ($r = 1;$r<=$_POST['event_date_repetition'];$r++){
				if ($r == 1){
					$start_date = $event_start_datexx;
					$end_date = $event_end_datexx;
					
				}else{
					$xrep = $r - 1;
					$timeStamp = strtotime($event_start_datexx);
					$timeStamp += 24 * 60 * 60 * ($xrep*7);
					$start_date = date('Y-m-d H:i:s',$timeStamp);
					$timeStampx = strtotime($event_end_datexx);
	...
	case 'EDIT':
		...
		header("Location: ".INDEX_PAGE."events&a=edit&success=true");
		break;
...
<div class="content-main wide65">
	...
	<div class="standard-form-instruction"><strong>Bemerkung</strong> <?php echo $req_fld?> Eingabe erforderlich</div>
    <?php } ?>
    <form action="<?php echo INDEX_PAGE . $page_option ?>" method="post" id="frm_<?php echo $page_name?>"  enctype="multipart/form-data">
        ...
        <fieldset class="standard-form">
            <legend>Einzelheiten</legend>
            <table class="form-table">            	
                <tr>
					<td class="key"><label for="category_id">Kategorie <?php echo $req_fld?></label></td>
					<?php if ( $is_editable_field ) { ?>
					<td>
						<?php
							$value_display['value'] = "category_id";
							$value_display['display'] = "category_name";
							$rs = $db->get_results("SELECT * FROM ".DB_TABLE_PREFIX."course_categories where section=2 ORDER BY category_name ASC");	
							echo $scaffold->dropdown_rs($rs,$value_display,"category_id",$category_id,"Bitte w&auml;hlen","style='width:195px;'");
						?>
                ...
                <tr>
                    <td class="key"><label for="event_title">Titel <?php echo $req_fld?></label></td>
                    <?php if ( $is_editable_field ) { ?>
                    <td>
				...
				<tr>
                    <td class="key"><label for="event_sub_title">Untertitel <?php echo $req_fld?></label></td>
                    <?php if ( $is_editable_field ) { ?>
                    <td>
				...
				<tr>
                    <td class="key"><label for="event_location">Ort  <?php echo $req_fld?></label></td>
					 <?php if ( $is_editable_field ) { ?>
					<td>
				...
				<tr>
                    <td class="key"><label for="event_description">Beschreibung </label></td>
                    <?php if ( $is_editable_field ) { ?>
                    <td>
					...
					$(document).ready(function()  {
						...
						$("#counter").append("<br />Sie haben <strong>"+  characters+"</strong> Zeichen &uuml;brig");
						$("#event_description").keyup(function(){
							if($(this).val().length > characters){
							$(this).val($(this).val().substr(0, characters));
							}
						var remaining = characters -  $(this).val().length;
						$("#counter").html("<br />Sie haben <strong>"+  remaining+"</strong> Zeichen &uuml;brig");
						if(remaining <= 10)
						{
				...
				<tr>
                    <td class="key"><label for="event_price">Preis </label></td>
					 <?php if ( $is_editable_field ) { ?>
					<td>
			...
			?>
                <tr>
                    <td class="key"><label for="event_start_date">Starten Sie Datum & Zeit <?php echo $req_fld?></label></td>
                    <?php if ( $is_editable_field ) { ?>
                    <td>
                    	<input type="text" name="event_start_date" id="event_start_date" style="width:45px;" maxlength="50" value="<?php if ($mode != "add"){echo htmlentities(date("j.n.y",strtotime($event_start_date)));}?>" /> 
                ...
                <tr>
                    <td class="key"><label for="event_end_date">End Datum & Zeit <?php echo $req_fld?></label></td>
                    <?php if ( $is_editable_field ) { ?>
                    <td>
                    	<input type="text" name="event_end_date" id="event_end_date" style="width:45px;" maxlength="50" value="<?php if ($mode != "add"){echo htmlentities(date("j.n.y",strtotime($event_end_date)));}?>" /> 
				...
				<tr>
                    <td class="key"><label for="event_link">Wiederholung  <?php echo $req_fld?></label></td>
					 <?php if ( $is_editable_field ) { ?>
					<td>
			...
			?>
				<tr>
                    <td class="key"><label for="event_link">Datum </label></td>
					<td>
						...
						<div id="system-message">
							<div class="info">
								<div class="message">Datum der Veranstaltung erfolgreich gel&ouml;scht!</div>
							</div>
						</div>
						<?php } ?>
						<table>
							...
							<?php
								$rowdatexctr = 0;
								$sqldates = mysql_query("select * from tbl_event_dates where eevent_id='$event_id' and section=1 order by date_id asc");
							...
							<tr>
								<td>
									...
									<input type="text" name="event_start_date<?php echo $rowdatexctr;?>" id="event_start_date<?php echo $rowdatexctr;?>" style="width:45px;" maxlength="50" value="<?php if ($mode != "add"){echo htmlentities(date("j.n.y",strtotime($event_start_date)));}?>" /> 
								...
								<td>
									<input type="text" name="event_end_date<?php echo $rowdatexctr;?>" id="event_end_date<?php echo $rowdatexctr;?>" style="width:45px;" maxlength="50" value="<?php if ($mode != "add"){echo htmlentities(date("j.n.y",strtotime($event_end_date)));}?>" /> 
									...
									<a href="index.php?option=events-m&mode=<?php echo $mode;?>&id=<?php echo $event_id;?>&action=delete&delid=<?php echo $rowdates['date_id'];?>"><img src="images/x.png" border="0"/></a>
									<?php } ?>
								</td>
							</tr>
							<script>
			...
			?>
				<tr>
                    <td class="key" valign="top"><label for="image">Bild </label></td>
                    <?php if ( $is_editable_field ) { ?>
                    <td>
						<?php 
						if ($mode != "add"){
				...
				<input type="hidden" name="event_is_active" id="event_is_active" value="1" />
				
				<?php if ($mode != "add"){
				?>
				<tr>
                    <td class="key"><label for="event_end_date">Erstellungsdatum</label></td>
					...
					?>
						<tr>
                    <td class="key"><label for="event_end_date">Aktualisiertes Datum</label></td>
        ...
        <?php if ( $mode != 'delete' ) { 
			if ($mode =="edit"){
				...
				$button = "Neuen Termin hinzuf&uuml;gen";
			}
		?>       
        <div class="standard-form-buttons">
			<input class="button" name="Submit" id="Submit" type="submit" value="<?php echo $button?>">
            <?php if ( $is_editable_field ) { ?>
            &nbsp;&nbsp;<input class="button" name="btnCancel" id="btnCancel" type="button" value="Abbrechen">
            <?php } ?>
        </div>
        <?php } ?>
