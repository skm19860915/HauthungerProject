...
<div id="custom-search-form" class="form-search">
		<strong>Stichwort</strong>&nbsp;&nbsp;<input type="text" name="search_keyword" id="search_keyword">
		<?php
		$options_col = array('Titel', 'Untertitel','Ort');
		...
		?> 
		<input class="button-search" type="button" value="Suchen" id="btn_customsearch">
  	    <input class="button-search" type="button" name="btn_reset" id="btn_reset" value="L&ouml;schen" />
</div>
</form>
<div class="clr"></div>
...
<div id="system-message">
    <div class="info">
        <div class="message"><?php echo $helper->operation_msg($_GET['a'], $_GET['success'], "Veranstaltung")?></div>
    </div>
</div>
<?php } ?>
...
<script type="text/javascript">
	...
	({
		...
		colModel : 
			...
			{display: 'Kategorie', name : 'cat.category_name', width : 120, sortable : true, align: 'left'},
			{display: 'Titel', name : 'event_title', width : 200, sortable : true, align: 'left'},
			{display: 'Untertitel', name : 'event_sub_title', width : 120, sortable : true, align: 'left'},
			{display: 'Beginn', name : 'event_start_date', width : 85, sortable : false, align: 'right'},
			{display: 'Ende', name : 'event_end_date', width : 85, sortable : false, align: 'left'},
			{display: 'Ort', name : 'event_location', width : 125, sortable : true, align: 'left'}/*,
			...
			if($('.trSelected',grid).length>0)
			{
				if(confirm('Delete ' + $('.trSelected',grid).length + ' record(s)?'))
				{
					...
					({
						type: "POST",
						dataType: "json",
						url: "<?php echo PATH_COMPONENTS?>delete.php?tn=<?php echo urlencode($crypt->encrypt('events'))?>&fn=<?php echo urlencode($crypt->encrypt('event_id'))?>",
						data: "items="+itemlist,
