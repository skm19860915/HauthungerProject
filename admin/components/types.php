...
<div id="system-message">
    <div class="info">
        <div class="message"><?php echo $helper->operation_msg($_GET['a'], $_GET['success'], "Type")?></div>
    </div>
</div>
<?php } ?>
...
<script type="text/javascript">
	...
	({
		...
		colModel : 
			...
			{display: 'Type Name', name : 'category_name', width : 180, sortable : true, align: 'left'},
			{display: 'Color', name : 'category_text_color', width : 120, sortable : true, align: 'left'}
			],
		buttons : 
			[
		...
		sortname: "<?php echo $fg_types['sortname']?>",
		sortorder: "<?php echo $fg_types['sortorder']?>",
		usepager: true,
		useRp: true,
		rp: <?php echo $fg_types['rp']?>,
		rpOptions: <?php echo $fg_types['rpOptions']?>,
		showTableToggleBtn: <?php echo $fg_types['showTableToggleBtn']?>,
		width: '<?php echo $fg_types['width']?>',
		height: 'auto',
		resizable: <?php echo $fg_types['resizable']?>,
			...
			if($('.trSelected',grid).length>0)
			{
				if(confirm('Delete ' + $('.trSelected',grid).length + ' record(s)?'))
				{
					...
					({
						type: "POST",
						dataType: "json",
						url: "<?php echo PATH_COMPONENTS?>delete.php?tn=<?php echo urlencode($crypt->encrypt('course_categories'))?>&fn=<?php echo urlencode($crypt->encrypt('category_id'))?>",
						data: "items="+itemlist,
