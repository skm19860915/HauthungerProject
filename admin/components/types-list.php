...
session_start();
$table_name = DB_TABLE_PREFIX."course_categories";
$page = $_POST['page'];
$rp = $_POST['rp'];
$sortname = $_POST['sortname'];
$sortorder = $_POST['sortorder'];

if (!$sortname) $sortname = $fg_types['sortname'];
if (!$sortorder) $sortorder = $fg_types['sortorder'];
...
if ( ($search_keyword != '') || ($columns != '') ) {
	if ( $columns != '' ) {
		...
		$field_names = array('category_name', 'category_text_color');
	}	
	$where = $sql_helper->where_like($field_names, $search_keyword);
	$where += " and section is not null";
}else{
	$where = " where section is not null";
}
...
$operation_url = INDEX_PAGE.'types-m&mode=';
$rc = false;	
while ($row = mysql_fetch_array($result))
{
	...
	$json .= ",'".$string->grid_safe("#".$row['category_text_color'])."']";
	$json .= "}";
	$rc = true;		
}
$json .= "]\n";
