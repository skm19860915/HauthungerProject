...
$button = $helper->button_val($mode, "Course");
...
$tablename = DB_TABLE_PREFIX.'courses';
...
if ( $form_action != '' ) {
	...
	$delctr = 0;
	$deldatefrm = mysql_query("select * from tbl_event_dates where eevent_id='$event_id' and section=2 order by date_id asc");
	...
	$_POST['event_start_date'] = date("Y-m-d H:i:s",strtotime($eventstartdate . ' ' . $event_start_time));
	$_POST['event_end_date'] = date("Y-m-d H:i:s",strtotime($eventenddate . ' ' . $event_end_time));
...
switch ($form_action)
{
	case 'ADD':		
		...
		if ($id > 0){
			...
			for ($r = 1;$r<=$_POST['event_date_repetition'];$r++){
				if ($r == 1){
					...
					$timeStampx = strtotime($_POST["event_end_date"]);
		...
		header("Location: ".INDEX_PAGE."courses&a=add&success=".$result);
		break;
	...
	case 'EDIT':
		...
		header("Location: ".INDEX_PAGE."courses&a=edit&success=true");
		break;
	...
	case 'DELETE':
		if ( (strtoupper($_POST["Delete"]) == 'YES')) {
			...
			header("Location: ".INDEX_PAGE."courses&a=delete&success=".$result);
		} elseif ( strtoupper($_POST["Delete"]) == 'NO' ) {
			header("Location: ".INDEX_PAGE."event");
		} else { 
			header("Location: ".INDEX_PAGE."courses-m&id=".$event_id);
		}				
		break;
	
	case 'VIEW':
		header("Location: ".INDEX_PAGE."courses");
		break;
...
$record = $sql_helper->cget_row(DB_TABLE_PREFIX."courses", "event_id = '$event_id'") ;
...
jQuery.validator.addMethod("complete_url", function(val, elem) {
	...
	$('#btnCancel').click(function () {
		location.href = '<?php echo INDEX_PAGE."courses"?>';
	});
...
<?php if ( $mode == 'delete' ) { ?>
	...
	<div id="system-message">
		...
		<div class="alert">
			<div class="message">
			<?php echo CONFIRM_DELETE . "course" ?>?&nbsp;&nbsp;
    ...
    <form action="<?php echo INDEX_PAGE . $page_option ?>" method="post" id="frm_<?php echo $page_name?>"  enctype="multipart/form-data">
        ...
        <fieldset class="standard-form">
            ...
            <table class="form-table">            	
                <tr>
					<td class="key"><label for="category_id">Category <?php echo $req_fld?></label></td>
					<?php if ( $is_editable_field ) { ?>
					<td>
						<?php
							$value_display['value'] = "category_id";
							$value_display['display'] = "category_name";
							$rs = $db->get_results("SELECT * FROM ".DB_TABLE_PREFIX."course_categories where section is null ORDER BY category_name ASC");	
				...
				<tr>
					...
					<td>
						...
						<table>
							...
							<?php
								$rowdatexctr = 0;
								$sqldates = mysql_query("select * from tbl_event_dates where eevent_id='$event_id' and section=2 order by date_id asc");
							...
							<tr>
								...
								<td>
									...
									<a href="index.php?option=courses-m&mode=<?php echo $mode;?>&id=<?php echo $event_id;?>&action=delete&delid=<?php echo $rowdates['date_id'];?>"><img src="images/x.png" border="0"/></a>
									<?php } ?>
								</td>
							</tr>
							<script>
