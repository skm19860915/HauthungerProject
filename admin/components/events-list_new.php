...
session_start();
$table_name = DB_TABLE_PREFIX."events";
$page = $_POST['page'];
$rp = $_POST['rp'];
...
if ( ($search_keyword != '') || ($columns != '') ) {
	...
	$wherex = $sql_helper->where_like($field_names, $search_keyword);
	$where .= " and edates.section=1";
}else{
	$where .= " where edates.section=1";
	$wherex .= " where edates.section=1";
}
$sql = "SELECT *,MIN(start_date) as event_start_date, MAX(end_date) as event_end_date FROM $table_name e 
	inner join tbl_course_categories cat on 
...
$total = $sql_helper->get_var("SELECT count(*) as total FROM $table_name e inner join tbl_course_categories cat on 
	e.category_id=cat.category_id $wherex $sort") ;
...
while ($row = mysql_fetch_array($result))
{
	...
	$json .= ",'".($row['category_name']=="Bassersdorf"?"Basel":$row['category_name'])."'";
	// $json .= ",'".$string->grid_safe($row['event_title'])."'";
	$json .= ",'".$string->grid_safe($row['event_title']=="Bassersdorf"?"Basel":$row['event_title'])."'";
	...
	$json .= ",'".$string->grid_safe($row['event_location']=="Bassersdorf"?"Basel":$row['event_location'])."'";
	$json .= ",'".$active."']";
	$json .= "}";
	$rc = true;		
}
