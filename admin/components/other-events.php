...
<div id="custom-search-form" class="form-search">
		...
		<?php
		$options_col = array('Event Title', 'Event Sub-title','Event Location');
...
<div id="system-message">
    <div class="info">
        <div class="message"><?php echo $helper->operation_msg($_GET['a'], $_GET['success'], "Other Date")?></div>
    </div>
</div>
<?php } ?>
...
<script type="text/javascript">
	...
	({
		...
		colModel : 
			...
			{display: 'Category', name : 'cat.category_name', width : 120, sortable : true, align: 'left'},
			{display: 'Title', name : 'event_title', width : 150, sortable : true, align: 'left'},
			{display: 'Sub-Title', name : 'event_sub_title', width : 120, sortable : true, align: 'left'},
			{display: 'Start', name : 'event_start_date', width : 85, sortable : false, align: 'right'},
			{display: 'End', name : 'event_end_date', width : 85, sortable : false, align: 'left'},
			{display: 'Location', name : 'event_location', width : 125, sortable : true, align: 'left'},
			...
			if($('.trSelected',grid).length>0)
			{
				if(confirm('Delete ' + $('.trSelected',grid).length + ' record(s)?'))
				{
					...
					({
						type: "POST",
						dataType: "json",
						url: "<?php echo PATH_COMPONENTS?>delete.php?tn=<?php echo urlencode($crypt->encrypt('other_events'))?>&fn=<?php echo urlencode($crypt->encrypt('event_id'))?>",
						data: "items="+itemlist,
